...
<style>
    ...
    .btn-detail, .btn-print, .btn-delete {
        display: inline-flex;
        align-items: center;
        justify-content: center;
        width: 38px;
    ...
    .btn-detail:hover, .btn-print:hover, .btn-delete:hover {
                    ...
                    <tr id="row-<?= $l['id'] ?>">
                        ...
                        <td data-label="Aksi">
                            <div class="action-buttons">
                                ...
                                <a href="<?= base_url('laporan/print/' . $l['id']) ?>" target="_blank" class="btn-print" title="Print">
                                    <i class="fas fa-print"></i>
                                </a>
                                <button type="button" class="btn-delete" onclick="confirmDelete(<?= $l['id'] ?>, '<?= esc($l['nama_penyewa']) ?>')" title="Hapus">
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="7" class="text-center py-5">
                            ...
                            <a href="<?= base_url('sewa/create') ?>" class="btn btn-primary mt-2">
                                <i class="fas fa-plus me-2"></i>Buat Penyewaan Baru
                            </a>
                        </td>
                    </tr>
                <?php endif; ?>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body text-center">
                ...
                <p class="text-muted mb-0">Data laporan dari penyewa <strong id="deleteNamaPenyewa"></strong> akan dihapus secara permanen.</p>
...
function confirmDelete(id, namaPenyewa) {
    deleteId = id;
    document.getElementById('deleteNamaPenyewa').innerText = namaPenyewa;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('confirmDeleteBtn').addEventListener('click', function() {
        if (deleteId) {
            ...
            fetch(`<?= base_url('laporan/delete') ?>/${deleteId}`, {
                method: 'DELETE',
                headers: {
                ...
                if (data.success) {
                    ...
                    if (row) {
                        ...
                        setTimeout(() => {
                            ...
                            // Jika tidak ada data, reload halaman
...
function updateStatistics() {
    const remainingRows = document.querySelectorAll('tbody tr:not(.no-data)').length;
    const totalLaporanSpan = document.querySelector('.stat-card:first-child .stat-info p');
    const recordBadge = document.querySelector('.record-badge');
    const tableTitleSmall = document.querySelector('.table-title small');
    
    if (totalLaporanSpan) {
        totalLaporanSpan.textContent = remainingRows;
    }
    
    if (recordBadge) {
        recordBadge.innerHTML = `<i class="fas fa-file"></i> ${remainingRows} Records <i class="fas fa-sync-alt" style="margin-left: 8px; animation: spin 2s linear infinite;"></i>`;
    }
    
    if (tableTitleSmall) {
        tableTitleSmall.innerHTML = `<i class="fas fa-info-circle"></i> Total ${remainingRows} laporan tersedia di sistem`;
    }
    
    const infoText = document.querySelector('.info-text span');
    if (infoText) {
        infoText.innerHTML = `Menampilkan <strong>${remainingRows}</strong> laporan dari sistem`;
    }
}
