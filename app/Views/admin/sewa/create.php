    ...
    <div class="form-container">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h4 class="fw-bold text-dark mb-0">
                <i class="bi bi-calendar-check me-2 text-primary"></i> Form Penyewaan
            </h4>
        ...
        <div class="card">
            <div class="card-header">
                <h5><i class="bi bi-file-text me-2 text-primary"></i> Data Penyewaan</h5>
            </div>
            <div class="card-body p-4">
                <form action="<?= base_url('sewa/store') ?>" method="post" id="formSewa">
                    <?= csrf_field() ?>
                    
                    <div class="mb-4">
                        <label class="form-label required">Nama Penyewa</label>
                        <input type="text" name="nama_penyewa" class="form-control" placeholder="Masukkan nama penyewa" value="<?= old('nama_penyewa') ?>" required>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered" id="tableDetail">
                                ...
                                <tbody></tbody>
                                <tfoot><tr class="table-secondary"><td colspan="4" class="text-end fw-bold">Total Harga Sewa:</td><td colspan="2" class="fw-bold" id="grandTotalDisplay">Rp 0</td></tr></tfoot>
                            </table>
                        </div>
                    ...
                    <div class="row mb-4">
                        <div class="col-md-6">
                            <div class="diskon-box">
                                ...
                                <div class="row g-2">
                                    ...
                                    <div class="col-md-6">
                                        <input type="number" id="nilai_diskon" class="form-control" placeholder="Masukkan diskon" min="0" value="0">
                                    </div>
                                </div>
                        ...
                        <div class="col-md-6">
                            <div class="total-setelah-diskon">
                                ...
                                <h4 class="text-success" id="totalSetelahDiskonDisplay">Rp 0</h4>
                                <input type="hidden" name="diskon" id="diskon_nominal" value="0">
                                <input type="hidden" name="diskon_persen" id="diskon_persen_val" value="0">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="deskripsi" class="form-control" rows="3" placeholder="Masukkan deskripsi keperluan sewa (opsional)"><?= old('deskripsi') ?></textarea>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="date" name="checkin" id="checkin" class="form-control" value="<?= old('checkin') ?>" required>
                        <div class="form-text text-muted">Tanggal mulai penyewaan</div>
                    </div>

                    <div class="mb-4">
                        <label class="form-label required">Check Out</label>
                        <input type="date" name="checkout" id="checkout" class="form-control" value="<?= old('checkout') ?>" required>
                        <div class="form-text text-muted">Tanggal selesai penyewaan</div>
                    </div>

                    <div class="mb-4">
                        <label class="form-label">Lokasi / Venue</label>
                        <input type="text" name="lokasi" class="form-control" placeholder="Masukkan lokasi atau venue" value="<?= old('lokasi') ?>">
                        <div class="form-text text-muted">Lokasi acara atau venue</div>
                    </div>

                    <hr class="my-4">
                    <div class="d-flex justify-content-end gap-2">
                        <a href="<?= base_url('barang') ?>" class="btn btn-secondary"><i class="bi bi-arrow-left me-2"></i>Batal</a>
                        <button type="submit" class="btn btn-primary" id="btnSubmit"><i class="bi bi-save me-2"></i>Simpan Sewa</button>
                    </div>
                </form>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderTable() {
        ...
        if (checkin.value && checkout.value) {
            ...
            hari = Math.ceil((tgl2 - tgl1) / (1000 * 60 * 60 * 24));
            if (hari <= 0) hari = 1;
        }
        ...
        detail.forEach((item, index) => {
            ...
            if (item.kategori_sewa === 'harian') {
                subtitle = `(${subtotalPerHari.toLocaleString('id-ID')}/hari × ${hari} hari)`;
            } else {
                subtitle = `(Event - langsung)`;
            }
            ...
            tbody.innerHTML += `
                <tr>
                    ...
                    <td>
                        <span class="badge ${item.kategori_sewa === 'harian' ? 'badge-harian' : 'badge-event'} px-3 py-2">
                            <i class="bi bi-${item.kategori_sewa === 'harian' ? 'calendar-week' : 'star'} me-1"></i>
                            ${item.kategori_sewa === 'harian' ? 'Harian' : 'Event'}
                        </span>
                    </td>
                    <td>${item.jumlah}</td>
